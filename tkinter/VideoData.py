...
class VideoData:
    ...
    def get_image_fname( self, index ):
        fname =  self.extract_path_segment(self.table.at[index, 'screenshot_file'])
        return os.path.join(self.folder, fname)

    def get_audio_fname( self, index ):
        fname = self.extract_path_segment(self.table.at[index, 'audio_file'])
        return os.path.join(self.folder, fname)
    
    def save_parquets(self, fname):
        self.table.to_parquet(fname)
        print(f"Table saved to {fname}.")
