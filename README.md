...
确认了scixing在代码中用的模型是在3000个中国人上训练的一个模型
...
<p align="center">
    <img src="figures/cluster_result_vs_confidence.png" height="400">
</p>

我在4集亮剑(12,13,15,20)上进行置信度回归，用音频+图片特征信息 去回归 聚类的置信度

然后用一集亮剑（10）和一集haruhi做测试，看如果用特定置信度以上的数据去做聚类

边级别的准确率是否会提升。目前从亮剑来看是有明显的提升的，haruhi上至少没有明显的下降（都跨语种了）

## 新标注流的设计

我们要想一下新的标注流

目前我们有音频相似度，和置信度两个工具。并且实践下来之前的csv标注的效率还可以
