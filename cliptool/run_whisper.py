...
class Video2Subtitles(object):
    def __init__(self):
        MODEL_WHISPER = "large-v1"  # or your local model_path
    ...
    # 写入成csv格式 格式为 dialogues,start,end
    def write_csv(self, transcript: Iterator[dict], file: TextIO):
        count = 0
        for segment in transcript:
            count += 1
            print(f'{self.trad2simp(segment["text"]).replace(",", "，").strip()},{segment["start"]},{segment["end"]}', file=file, flush=True)
